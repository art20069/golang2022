...
// SetupProductAPI - call this method to setuup product route group //
...
func getProduct(c *gin.Context) {
	...
	if keyword != "" {
		...
		// db.GetDB().Find(&product)
		db.GetDB().Order("id asc").Find(&product)
	}
	c.JSON(200, product)
}
...
func getProductByID(c *gin.Context) {
	var product model.Product
	db.GetDB().Where("id =?", c.Param("id")).Find(&product)
	c.JSON(200, product)
}
